    ...
    while not Player.IsGhost and Player.GetRealSkillValue( 'Animal Taming' ) < Player.GetSkillCap( 'Animal Taming' ):
        ...
        if tameOngoing:
            if ( Journal.SearchByName( 'It seems to accept you as master.', animalBeingTamed.Name ) or
                ...
                Items.UseItem(0x40042457)
                Gumps.WaitForGump(4186437632, 10000)
                if Player.GetRealSkillValue( 'Animal Taming' ) < 60.0:
                    Gumps.SendAction(4186437632, 6)
                if Player.GetRealSkillValue( 'Animal Taming' ) < 66.0:
                    Gumps.SendAction(4186437632, 7)
                if Player.GetRealSkillValue( 'Animal Taming' ) < 71.0:
                    Gumps.SendAction(4186437632, 8)
                if Player.GetRealSkillValue( 'Animal Taming' ) < 81.0:
                    Gumps.SendAction(4186437632, 9)
                if Player.GetRealSkillValue( 'Animal Taming' ) < 91.0:
                    Gumps.SendAction(4186437632, 10)
                if Player.GetRealSkillValue( 'Animal Taming' ) < 101.0:
                    Gumps.SendAction(4186437632, 11)
                if Player.GetRealSkillValue( 'Animal Taming' ) < 120.0:
                    Gumps.SendAction(4186437632, 12)
